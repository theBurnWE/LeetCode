...
_JAR_KEYWORDS=spbootVueFrame.jar
APP_NAME=spbootVueFrame
...
case "$1" in
  ...
  start)
   ...
   nohup $JAVA_HOME/bin/java -jar -server $JAVA_OPTIONS_INITIAL $JAVA_OPTIONS_MAX $JAVA_OPTIONS_PERMSIZE $JAVA_OPTIONS_MAX_PERMSIZE $_JAR_KEYWORDS --spring.config.location=$APPLICATION_FILE > output 2>&1 &
