...
public class CwsUtil {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CwsUtil.class);
    /**
    ...
    static String calculateStringToSignV2(URI serviceUri,
        ...
        data.append(CwsUtil.urlEncode(uri, true));
        data.append("\n");
        Map<String, String> sorted = new TreeMap<String, String>();
        sorted.putAll(parameters);
        Iterator<Entry<String, String>> pairs = sorted.entrySet()
                                                      .iterator();
        while (pairs.hasNext()) {
            Entry<String, String> pair = pairs.next();
            String key = pair.getKey();
            data.append(CwsUtil.urlEncode(key, false));
            data.append("=");
            String value = pair.getValue();
            data.append(CwsUtil.urlEncode(value, false));
            if (pairs.hasNext()) {
                data.append("&");
            }
        }
    ...
    public static String sign(String data, String key, String algorithm) {
        try {
            ...
            md.update((data + key).getBytes(CwsUtil.DEFAULT_ENCODING));
            byte b[] = md.digest();
            int i;
    ...
    public static String generateUUID() {
        return new String(UUID.randomUUID().toString());//Hex.encodeHex(org.apache.commons.id.uuid.UUID.randomUUID()
        //                                           .getRawBytes()));
    }
